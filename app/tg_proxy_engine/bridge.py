...
# Throttle state: key -> last logged timestamp.
# Prevents the log from being flooded with identical error messages when CF
# proxy returns 429 (rate limited) or times out on every connection attempt.
_throttle_last_log: dict = defaultdict(float)


def _throttled_log(msg: str, *args, min_interval: float = 60.0, key: str = ""):
    """Log a debug message at most once per ``min_interval`` seconds.

    Fallback errors (CF proxy 429, TCP timeout) are logged at DEBUG level
    so they DON'T appear in the GUI log (which is set to INFO). Telegram
    works fine through the primary WebSocket path for most DCs — these
    fallback failures only affect specific DCs that are blacklisted, and
    the user doesn't need to see them."""
    now = time.monotonic()
    last = _throttle_last_log.get(key, 0.0)
    if now - last >= min_interval:
        _throttle_last_log[key] = now
        log.debug(msg, *args)

def _cf_failure_cooldown(exc) -> float:
    """Return a per-domain cooldown for CF fallback errors.

    HTTP 429 is a real Cloudflare rate-limit signal, not a normal transient
    failure. Retrying immediately on every Telegram connection worsens the
    limit and creates log storms, so we cool that domain down longer. Timeouts
    get a shorter cooldown to avoid slow repeated attempts while still allowing
    quick recovery if the network improves.
    """
    status = getattr(exc, "status_code", None)
    text = repr(exc)
    if status == 429 or "HTTP 429" in text or "Too Many Requests" in text:
        return 10 * 60.0
    if isinstance(exc, TimeoutError) or "TimeoutError" in text:
        return 90.0
    return 30.0
...
async def do_fallback(reader, writer, relay_init, label,
    ...
    for method in methods:
        if method == 'cf_worker' and fallback_dst:
            ...
            log.debug("[%s] DC%d%s -> TCP fallback to %s:443",
...
async def _cfproxy_worker_fallback(reader, writer, relay_init, label,
    ...
    for worker_domain in worker_domains:
        ...
        if ws:
            ...
            log.debug("[%s] DC%d%s -> trying CF worker %s for %s",
...
async def _cfproxy_fallback(reader, writer, relay_init, label,
    ...
    _throttled_log(
        "[%s] DC%d%s -> trying CF proxy",
        label, dc, media_tag,
        min_interval=60.0,
        key=f"trying_cfproxy_dc{dc}",
    )
    ...
    for base_domain in balancer.get_domains_for_dc(dc):
        ...
        try:
            ...
            cooldown = _cf_failure_cooldown(exc)
            balancer.mark_domain_failed(dc, base_domain, cooldown)
            # Throttle: only log CF proxy errors at most once per 30 seconds
            # per DC. Cloudflare 429 (rate limit) and TimeoutError are
            # expected when CF is overloaded — logging every single failure
            # floods the GUI log with hundreds of identical lines.
            _throttled_log(
                "[%s] DC%d%s CF proxy failed: %s (domain %s cooldown %.0fs)",
                label, dc, media_tag, repr(exc), base_domain, cooldown,
                min_interval=30.0,
                key=f"cfproxy_dc{dc}",
            )

    if ws is None:
        cooled = balancer.cooldown_count(dc)
        total = len(getattr(balancer, "domains", []) or [])
        if total and cooled >= total:
            _throttled_log(
                "[%s] DC%d%s all CF proxy domains are cooling down after rate-limit/timeout; trying next fallback",
                label, dc, media_tag,
                min_interval=60.0,
                key=f"cfproxy_all_cooldown_dc{dc}",
            )
        return False

    if chosen_domain and balancer.update_domain_for_dc(dc, chosen_domain):
        log.debug("[%s] Switched active CF domain", label)
