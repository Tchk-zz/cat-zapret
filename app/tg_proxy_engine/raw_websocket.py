...
# Keep certificate validation enabled. We connect to a pinned/selected IP but
# pass the real WebSocket domain as server_hostname, so TLS SNI and hostname
# verification both still work while preventing MITM on the Telegram WS tunnel.
...
class RawWebSocket:
    ...
    # Bound memory usage before allocating/reading an untrusted WS frame.
        ...
        while not self._closed:
            ...
            if opcode in (self.OP_CONT, 0x1, self.OP_BINARY):
                ...
                if len(self._frag) > self.MAX_MESSAGE_LEN:
                    self._frag.clear()
                    raise ConnectionError("WebSocket message exceeds safety limit")
                if not fin:
                    continue
