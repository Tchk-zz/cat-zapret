...
WS_FAIL_TIMEOUT = 2.0
BAD_HANDSHAKE_LOG_INTERVAL = 60.0
ws_blacklist: Set[str] = set()
dc_fail_until: Dict[str, float] = {}
_last_bad_handshake_log = 0.0
_bad_handshake_suppressed = 0


def _log_bad_handshake(label: str) -> None:
    """Warn about wrong-secret/proto handshakes without flooding the GUI log.

    This usually happens when Telegram Desktop still uses an old tg://proxy
    secret after the user regenerated it, or when some local process probes the
    MTProto port with non-MTProto bytes. Telegram can reconnect many times with
    new localhost ports, so logging every attempt is noisy and not actionable.
    """
    global _last_bad_handshake_log, _bad_handshake_suppressed
    now = time.monotonic()
    if now - _last_bad_handshake_log >= BAD_HANDSHAKE_LOG_INTERVAL:
        suffix = ""
        if _bad_handshake_suppressed:
            suffix = f" (suppressed {_bad_handshake_suppressed} similar attempts)"
            _bad_handshake_suppressed = 0
        _last_bad_handshake_log = now
        log.warning(
            "[%s] bad handshake (wrong secret/proto; update Telegram proxy link if this repeats)%s",
            label, suffix,
        )
    else:
        _bad_handshake_suppressed += 1
...
async def _handle_client(reader, writer, secret: bytes):
    ...
    try:
        ...
        if result is None:
            ...
            _log_bad_handshake(label)
            # Close immediately. Reading until EOF lets a broken/stale client
            # keep the handler alive and produce repeated reconnect noise; a
            # wrong-secret/proto connection cannot be recovered in-place.
            try:
                clt_writer.close()
                await clt_writer.wait_closed()
            except Exception:
                pass
            return
        ...
        if dc not in proxy_config.dc_redirects or dc_key in ws_blacklist:
            if dc not in proxy_config.dc_redirects:
                from .bridge import _throttled_log
                _throttled_log(
                    "[%s] DC%d not in config -> fallback",
                    label, dc,
                    min_interval=60.0,
                    key=f"dc_not_config_{dc}",
                )
            else:
                from .bridge import _throttled_log
                _throttled_log(
                    "[%s] DC%d%s WS blacklisted -> fallback",
                    label, dc, media_tag,
                    min_interval=60.0,
                    key=f"ws_blacklisted_{dc_key}",
                )
            splitter = None
            try:
            ...
            if not ok:
                # Throttle: "no fallback available" fires on every connection
                # attempt when both CF proxy and TCP fallback fail (e.g. CF
                # is rate-limiting with 429 AND the direct IP is blocked).
                # Without throttling this floods the log.
                from .bridge import _throttled_log
                _throttled_log(
                    "[%s] DC%d%s no fallback available (CF proxy + TCP both failed)",
                    label, dc, media_tag,
                    min_interval=60.0,
                    key=f"no_fallback_dc{dc}",
                )
            return
        ...
        if ws is None:
            ...
            if ok:
                log.debug("[%s] DC%d%s fallback closed",
                         label, dc, media_tag)
            return
...
def main():
    ...
    if args.log_file:
        from .logging_setup import build_log_handler
